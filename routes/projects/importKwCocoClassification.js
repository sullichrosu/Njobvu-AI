...
// Strangler-pattern sibling of importKwCoco.js: handles the classification
// branch of the KW Coco Archive creation flow in isolation so the existing
// detection handler stays untouched. Each image here gets a single
// per-image class label instead of per-object bounding boxes.
const importKwCocoClassification = async (req, res) => {
    try {
        ...
        const args = ['-u', scriptPath, '-i', unzippedPath, '-o', projectPath, '-d', projectName, '-r', 'coco_class'];
        if (modelPathVal) {
        ...
        pythonProcess.on('close', async (code) => {
            if (code !== 0) {
                console.error(`KW Coco classification import python script exited with code ${code}:`);
                console.error(stderr);
                return res.status(500).json({ success: false, message: stderr || 'Error parsing KW Coco archive' });
            }

            try {
                const projectDescription = "KW Coco Imported Project (Classification)";
                const autoSave = 1;
                ...
                res.json({ success: true, message: 'KW Coco classification import process completed.', output: stdout });
            } catch (dbError) {
        ...
        console.error(err);
        res.status(500).json({ success: false, message: 'KW Coco classification import process failed' });
    }
};

module.exports = importKwCocoClassification;

...
async function getInferencePage(req, res) {
    ...
    try {
        ...
        global.logger.error("Error loading inference page:", err);
    ...
    const projectDir = fsPath.join(publicPath, "public", "projects", `${admin}-${PName}`);
    const trainingPath = fsPath.join(projectDir, "training");
    const weightsPath = fsPath.join(trainingPath, "weights");
    const inferencePath = fsPath.join(projectDir, "inference");
    const inferenceUploadPath = fsPath.join(inferencePath, "uploads");
    const logPath = fsPath.join(trainingPath, "logs/");
    const trnLogPath = fsPath.join(trainingPath, "logs/");
    const infLogPath = fsPath.join(inferencePath, "logs/");
    const infUploadPath = fsPath.join(inferencePath, "uploads/");
    const pythonPath = fsPath.join(trainingPath, "python");
    const pythonPathFile = fsPath.join(trainingPath, "Paths.txt");
    const darknetPathFile = fsPath.join(trainingPath, "darknetPaths.txt");
    const weightsOutput = fsPath.join(trainingPath, "train", "weights", "best.pt");
    ...
    try {
        ...
        runs = (runs || []).filter((r) => {
            try {
                return fsObj.statSync(fsPath.join(logPath, r)).isDirectory();
            } catch (e) {
                return true;
            }
        });
    ...
    try {
        ...
        infRuns = (infRuns || []).filter((r) => {
            try {
                return fsObj.statSync(fsPath.join(infLogPath, r)).isDirectory();
            } catch (e) {
                return true;
            }
        });
    ...
    for (let i = 0; i < runs.length; i++) {
        ...
        const runDir = fsPath.join(logPath, `${runName}/`);
        ...
        try {
            logContents.push(fsObj.readFileSync(fsPath.join(runDir, `${runName}.log`), "utf8"));
        } catch (e) {
            logContents.push("");
        }
        ...
        if (errIdx >= 0) {
            ...
            try {
                errContents.push(fsObj.readFileSync(fsPath.join(runDir, logs[errIdx]), "utf8"));
            } catch (e) {
                errContents.push("");
            }
    ...
    for (let i = 0; i < infRuns.length; i++) {
        ...
        const runDirInf = fsPath.join(infLogPath, `${infRunName}/`);
        ...
        try {
            logContentsInf.push(fsObj.readFileSync(fsPath.join(runDirInf, `${infRunName}.log`), "utf8"));
        } catch (e) {
            logContentsInf.push("");
        }
        ...
        if (runTypePath >= 0) {
            try {
                const type = fsObj.readFileSync(fsPath.join(runDirInf, logsInf[runTypePath]), "utf8");
        ...
        if (errIdxInf >= 0 && doneIdxInf === -1) {
            ...
            try {
                errContentsInf.push(fsObj.readFileSync(fsPath.join(runDirInf, logsInf[errIdxInf]), "utf8"));
            } catch (e) {
                errContentsInf.push("");
            }

            for (let j = 0; j < logsInf.length; j++) {
                if (j === errIdxInf || isReservedInferenceFile(logsInf[j])) continue;
            ...
            for (let j = 0; j < logsInf.length; j++) {
                if (j === doneIdxInf || isReservedInferenceFile(logsInf[j])) continue;
            ...
            for (let j = 0; j < logsInf.length; j++) {
                if (isReservedInferenceFile(logsInf[j])) continue;
...
module.exports = getInferencePage;
